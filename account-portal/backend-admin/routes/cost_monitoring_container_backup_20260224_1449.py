...
def is_valid_project(project_name):
    ...
    if not project_name or project_name == "unknown":
        return False
    system_dirs = [".vscode-server", "miniconda3", "programs", "notebooks", "cost-agent", "mogam_project", "code", "ray", "notes", ".aws"]
    if project_name in system_dirs:
        return False
    if project_name.startswith("tmp"):
        return False
    if re.match(r"^\d{2}-\d{2}-\d{2}$", project_name):
        return False
    if re.match(r"^\d+$", project_name):
        return False
    if project_name.endswith("_홈"):
        return False
    if project_name.startswith("ms-"):
        return False
    return True
    ...
    try:
        ...
        for code, d in code_aggregated.items():
            projects.append({
                ...
                "users": sorted(list(set([resolve_username(u) for u in d["users"]])))
            })

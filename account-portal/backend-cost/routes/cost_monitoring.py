...
def is_valid_project(project_name):
    ...
    system_dirs = [
        ...
        'project', 'files', 'old_cds_bart_v2', 'materials'
    ]
...
def get_project_info(project_name):
    # 와일드카드 패턴 매칭 (*, ? 지원)
    import fnmatch
    for pattern, code in _mp.PROJECT_NAME_TO_CODE.items():
...
def get_cost_data(date):
    ...
    agent_rows = res_agent['ResultSet']['Rows'][1:]
    while 'NextToken' in res_agent:
        res_agent = athena_client.get_query_results(QueryExecutionId=r_agent['QueryExecutionId'], NextToken=res_agent['NextToken'])
        agent_rows.extend(res_agent['ResultSet']['Rows'])
    ...
    instance_project_data = defaultdict(lambda: defaultdict(lambda: {
        ...
        'cpu_weight': 0.0,
        'samples': 0,
        'users': defaultdict(lambda: {'samples': 0, 'weight': 0.0, 'gpu_weight': 0.0, 'cpu_weight': 0.0, 'gpu_count': 0})
    }))
    instance_total_weight = defaultdict(float)
    instance_total_gpu_weight = defaultdict(float)
    instance_total_cpu_weight = defaultdict(float)
    ...
    for row in agent_rows:
        ...
        if not project.startswith('_unknown_') and not is_valid_project(project):
            continue
        samples = int(vals[3])
        ...
        # 가중치 계산: GPU/CPU 분리
        # cpu_weight: 모든 프로세스의 CPU 사용량 기반
        cpu_w = samples * (max(avg_cpu_percent, 1.0) / 100.0)
        # gpu_weight: GPU 사용 프로세스만 (SUM(gpu_count)가 이미 샘플 반영된 값)
        gpu_w = float(gpu_count) if gpu_count > 0 else 0.0
        # 통합 weight (기존 호환용)
        if gpu_count > 0:
            effective_cpu = max(avg_cpu_percent, 10.0)
            weight = samples * (effective_cpu / 100.0) * (1.0 + gpu_count)
        else:
            weight = samples * (max(avg_cpu_percent, 1.0) / 100.0) * (1.0 + gpu_count)
        
        # 집계
        instance_total_weight[instance_id] += weight
        instance_total_gpu_weight[instance_id] += gpu_w
        instance_total_cpu_weight[instance_id] += cpu_w
        instance_project_data[instance_id][project]['weight'] += weight
        instance_project_data[instance_id][project]['gpu_weight'] += gpu_w
        instance_project_data[instance_id][project]['cpu_weight'] += cpu_w
        ...
        instance_project_data[instance_id][project]['users'][username]['gpu_weight'] += gpu_w
        instance_project_data[instance_id][project]['users'][username]['cpu_weight'] += cpu_w
    ...
    # 2.5. 에이전트 수집 실패 기간 보정 (P5, G4DN)
    _AGENT_FALLBACK = {
        'i-004e23df368dcb30e': {'project': 'P250001_LLM2', 'user': '1017',
            'dates': {  # P5: processes=0 기간
                '2026-02-02','2026-02-03','2026-02-04',
                '2026-02-16','2026-02-17','2026-02-18','2026-02-19',
                '2026-02-24','2026-02-25','2026-02-26','2026-02-27','2026-02-28',
                '2026-03-01','2026-03-02','2026-03-03','2026-03-04','2026-03-05'}},
        'i-074a73c3cf9656989': {'project': 'P240001_mRNA_MGCD', 'user': '1005',
            'dates': {  # jhhong 퇴사 전 (2/13 퇴사)
                '2026-02-01','2026-02-02',
                '2026-02-07','2026-02-08','2026-02-09','2026-02-10','2026-02-11',
                '2026-02-12'}},
        'i-074a73c3cf9656989_post': {'project': 'P240001_mRNA_MGCD', 'user': '1014',
            'instance_id': 'i-074a73c3cf9656989',
            'dates': {  # shlee2 인수 (jhhong 퇴사 후, 에이전트 고장 기간)
                '2026-02-13','2026-02-14','2026-02-15','2026-02-16',
                '2026-02-17','2026-02-18','2026-02-19','2026-02-20','2026-02-21',
                '2026-02-22','2026-02-23'}},
    }
    for fb_inst, fb_info in _AGENT_FALLBACK.items():
        real_inst = fb_info.get('instance_id', fb_inst)
        if date in fb_info['dates'] and real_inst not in instance_project_data and real_inst in instance_costs:
            cost = instance_costs[real_inst]
            proj = fb_info['project']
            uname = fb_info['user']
            gpu_ratio = INSTANCE_GPU_RATIO.get(real_inst, 0)
            gw = cost * gpu_ratio
            cw = cost * (1 - gpu_ratio)
            instance_project_data[real_inst][proj]['weight'] = gw + cw
            instance_project_data[real_inst][proj]['gpu_weight'] = gw
            instance_project_data[real_inst][proj]['cpu_weight'] = cw
            instance_project_data[real_inst][proj]['samples'] = 1
            instance_project_data[real_inst][proj]['users'][uname]['weight'] = gw + cw
            instance_project_data[real_inst][proj]['users'][uname]['gpu_weight'] = gw
            instance_project_data[real_inst][proj]['users'][uname]['cpu_weight'] = cw
            instance_project_data[real_inst][proj]['users'][uname]['samples'] = 1
            instance_total_weight[real_inst] = gw + cw
            instance_total_gpu_weight[real_inst] = gw
            instance_total_cpu_weight[real_inst] = cw

    # 3. 프로젝트별 비용 배분 - GPU/CPU 풀 분리 배분
    ...
    # 1차 배분: 인스턴스별 GPU풀/CPU풀 분리 → 각각 gpu_weight/cpu_weight 기반 배분
    for instance_id, projects in instance_project_data.items():
        ...
        gpu_pool = instance_cost * gpu_ratio
        cpu_pool = instance_cost * (1 - gpu_ratio)
        
        total_gw = instance_total_gpu_weight[instance_id]
        total_cw = instance_total_cpu_weight[instance_id]
        
        # 엣지 케이스: GPU/CPU 사용자 없으면 해당 풀은 unallocated로 유지
        if total_gw == 0:
            gpu_pool = 0
        if total_cw == 0:
            cpu_pool = 0
        ...
        for project, proj_data in projects.items():
            # GPU 풀 배분 (gpu_weight 기반, GPU 사용자만)
            proj_gpu = gpu_pool * (proj_data['gpu_weight'] / total_gw) if total_gw > 0 else 0
            # CPU 풀 배분 (cpu_weight 기반, 모든 사용자)
            proj_cpu = cpu_pool * (proj_data['cpu_weight'] / total_cw) if total_cw > 0 else 0
            proj_total = proj_gpu + proj_cpu
            
            instance_allocated[instance_id] += proj_total
            proj_costs[project]['total_cost'] += proj_total
            proj_costs[project]['gpu_cost'] += proj_gpu
            proj_costs[project]['cpu_cost'] += proj_cpu
            proj_costs[project]['samples'] += proj_data['samples']
            
            # 유저별 비용 배분 (프로젝트 내 gpu_weight/cpu_weight 비율)
            proj_total_gw = proj_data['gpu_weight']
            proj_total_cw = proj_data['cpu_weight']
            for username, user_data in proj_data['users'].items():
                proj_costs[project]['users'].add(username)
                
                user_gpu = proj_gpu * (user_data['gpu_weight'] / proj_total_gw) if proj_total_gw > 0 else 0
                user_cpu = proj_cpu * (user_data['cpu_weight'] / proj_total_cw) if proj_total_cw > 0 else 0
                user_total = user_gpu + user_cpu
                
                user_costs[username]['total_cost'] += user_total
                user_costs[username]['gpu_cost'] += user_gpu
                user_costs[username]['cpu_cost'] += user_cpu
                user_costs[username]['projects'].add(project)
    
    # 4. 인스턴스별 미배분 비용 재배분 (GPU/CPU 풀 분리)
    for instance_id, instance_cost in instance_costs.items():
        ...
        # 미배분 비용도 GPU/CPU 풀 비율로 분리
        gpu_ratio = INSTANCE_GPU_RATIO.get(instance_id, 0)
        unalloc_gpu = unallocated * gpu_ratio
        unalloc_cpu = unallocated * (1 - gpu_ratio)
        
        total_gw = sum(p['gpu_weight'] for p in projects.values())
        total_cw = sum(p['cpu_weight'] for p in projects.values())
        ...
        for project, proj_data in projects.items():
            add_gpu = unalloc_gpu * (proj_data['gpu_weight'] / total_gw) if total_gw > 0 else 0
            add_cpu = unalloc_cpu * (proj_data['cpu_weight'] / total_cw) if total_cw > 0 else 0
            add_total = add_gpu + add_cpu
            
            proj_costs[project]['total_cost'] += add_total
            proj_costs[project]['gpu_cost'] += add_gpu
            proj_costs[project]['cpu_cost'] += add_cpu
            
            proj_total_gw = proj_data['gpu_weight']
            proj_total_cw = proj_data['cpu_weight']
            for username, user_data in proj_data['users'].items():
                u_gpu = add_gpu * (user_data['gpu_weight'] / proj_total_gw) if proj_total_gw > 0 else 0
                u_cpu = add_cpu * (user_data['cpu_weight'] / proj_total_cw) if proj_total_cw > 0 else 0
                user_costs[username]['total_cost'] += u_gpu + u_cpu
                user_costs[username]['gpu_cost'] += u_gpu
                user_costs[username]['cpu_cost'] += u_cpu
                user_costs[username]['projects'].add(project)
    ...
    _result = {
        ...
        'total_cost_krw': round(total_cost * (get_exchange_rate(date) or 1440), 2),
        'mtd_cost': mtd_cost,
...
def get_daily_costs(date):
    ...
    req_date = _date.fromisoformat(date)
    if req_date > max_date:
        return jsonify({"error": "Data not available yet (48-hour delay policy)"}), 400
    try:
        ...
        for code, d in code_aggregated.items():
            ...
            projects.append({
                ...
                "total_cost_krw": round(d["total_cost"] * (get_exchange_rate(date) or 1440), 2),
                "gpu_cost_krw": round(d["gpu_cost"] * (get_exchange_rate(date) or 1440), 2),
                "cpu_cost_krw": round(d["cpu_cost"] * (get_exchange_rate(date) or 1440), 2),
                "users": sorted([resolve_username(u) for u in d["users"] if data.get('user_costs',{}).get(u,{}).get('total_cost',0) > 1])
            })
        # 프로젝트별 compute_nodes 매핑
        for p in projects:
            p['compute_nodes'] = {t: {'count': cn['count'], 'cost_usd': round(cn['cost'], 2), 'cost_krw': round(cn['cost'] * (get_exchange_rate(date) or 1440), 2)} for t, cn in data.get('compute_nodes', {}).items() if p['code'] in cn.get('projects', [])}
        ...
        for instance_id, cost in data.get('instance_costs', {}).items():
            ...
            if instance_id in INSTANCE_ORDER:
                ...
                instances[instance_id] = {
                    ...
                    'total_cost_krw': round(cost * (get_exchange_rate(date) or 1440), 2),
        ...
        total_cost_krw = round(data['total_cost'] * (get_exchange_rate(date) or 1440), 2)
        return jsonify({
            'date': date,
            ...
            'compute_nodes': {t: {'count': d['count'], 'cost_usd': round(d['cost'], 2), 'cost_krw': round(d['cost'] * (get_exchange_rate(date) or 1440), 2), 'projects': d.get('projects', [])} for t, d in data.get('compute_nodes', {}).items() if d['cost'] > 0.5},
            'validation': {
            ...
            'unallocated_cost_krw': round(total_cost_krw - sum(p.get('total_cost_krw', 0) for p in projects), 2),
            'agent_raw': agent_raw,
            ...
            'monthly_accumulated_krw': round(data.get('total_cost_krw', 0) + sum(_daily_cache.get(f'{date[:7]}-{d:02d}', {}).get('data', {}).get('total_cost_krw', 0) for d in range(1, int(date.split("-")[2]))), 2),
...
def get_finops_daily(date):
    try:
        ...
        proj_user_set = {p["project"]: set(p.get("users", [])) for p in projects}
...
def get_monthly_cost_data(year, month):
    ...
    total_cost = 0.0
    total_cost_krw = 0.0
    proj_agg = defaultdict(lambda: {'total_cost': 0, 'gpu_cost': 0, 'cpu_cost': 0, 'total_cost_krw': 0, 'gpu_cost_krw': 0, 'cpu_cost_krw': 0, 'users': set(), 'compute_nodes': {}})
    user_agg = defaultdict(lambda: {'total_cost': 0, 'gpu_cost': 0, 'cpu_cost': 0, 'projects': set()})
    inst_agg = {}
    cn_agg = defaultdict(lambda: {'count': 0, 'cost': 0.0, 'cost_krw': 0.0, 'projects': set()})
    unalloc_sum = 0.0
    ...
    for day in range(1, days_in_month + 1):
        ...
        if d < min_date:
            continue
        try:
            resp = requests.get(f'http://backend-cost:5000/api/cost-monitoring/daily-costs/{date_str}', timeout=30)
        ...
        total_cost_krw += d.get('total_cost_krw', 0)
        ...
        for p in d.get('projects', []):
            ...
            proj_agg[code]['total_cost_krw'] += p.get('total_cost_krw', 0)
            proj_agg[code]['gpu_cost_krw'] += p.get('gpu_cost_krw', 0)
            proj_agg[code]['cpu_cost_krw'] += p.get('cpu_cost_krw', 0)
        ...
        for iid, inst in d.get('instances', {}).items():
            ...
            inst_agg[iid]['total_cost_krw'] += inst.get('total_cost_krw', 0)
        ...
        for t, cn in d.get('compute_nodes', {}).items():
            ...
            cn_agg[t]['cost_krw'] += cn.get('cost_krw', 0)
    ...
    result = {
        'total_cost': total_cost, 'total_cost_krw': total_cost_krw,
    ...
    try:
        ...
        for code, d in data['proj_agg'].items():
            ...
            projects.append({
                ...
                'gpu_cost': round(d['gpu_cost'], 2),
                'cpu_cost': round(d['cpu_cost'], 2),
                'total_cost_krw': round(d.get('total_cost_krw', 0), 2),
                'gpu_cost_krw': round(d.get('gpu_cost_krw', 0), 2),
                'cpu_cost_krw': round(d.get('cpu_cost_krw', 0), 2),
        ...
        for iid, inst in data['inst_agg'].items():
            ...
            inst_copy['total_cost_krw'] = round(inst.get('total_cost_krw', 0), 2)
        ...
        for t, cn in data['cn_agg'].items():
            if cn['cost'] > 0.5:
                cn_out[t] = {'count': cn['count'], 'cost_usd': round(cn['cost'], 2), 'cost_krw': round(cn.get('cost_krw', 0), 2), 'projects': sorted(list(cn['projects']))}
        ...
        return jsonify({
            ...
            'total_cost': round(data['total_cost'], 2),
            'total_cost_krw': round(data.get('total_cost_krw', 0), 2),
            ...
            'unallocated_cost_krw': round(data.get('total_cost_krw', 0) - sum(p.get('total_cost_krw', 0) for p in projects), 2),
...
# ===== 스토리지 비용 배분 (엑셀 전용) =====
@cost_monitoring_bp.route('/storage-costs/<year>/<month>', methods=['GET'])
def get_storage_costs(year, month):
    import calendar, boto3, time
    from datetime import datetime, timedelta
    from utils.exchange_rate import get_exchange_rate
    ...
    try:
        athena = boto3.client('athena', region_name='us-west-2')
        def run_q(q):
            r = athena.start_query_execution(QueryString=q, QueryExecutionContext={'Database': 'cost_monitoring'}, ResultConfiguration={'OutputLocation': 's3://mogam-or-cur-stg/athena-results/'})
            ...
            if st != 'SUCCEEDED':
                return []
            return athena.get_query_results(QueryExecutionId=qid)['ResultSet']['Rows'][1:]
        
        # CUR: S3/FSx 월 비용
        rows = run_q(f"SELECT CASE WHEN product_product_name='Amazon Simple Storage Service' THEN 'S3' WHEN product_product_name LIKE '%FSx%' THEN 'FSx' END AS svc, SUM(CASE WHEN line_item_line_item_type='SavingsPlanCoveredUsage' THEN savings_plan_savings_plan_effective_cost WHEN line_item_line_item_type='SavingsPlanNegation' THEN 0 WHEN line_item_line_item_type='DiscountedUsage' THEN reservation_effective_cost ELSE line_item_unblended_cost END) FROM cur_database.mogam_hourly_cur WHERE year='{year}' AND month='{int(month)}' AND (product_product_name='Amazon Simple Storage Service' OR product_product_name LIKE '%FSx%') GROUP BY 1")
        s3_m = fsx_m = 0
        for r in rows:
            v = [c.get('VarCharValue','') for c in r['Data']]
            if v[0] == 'S3': s3_m = float(v[1] or 0)
            elif v[0] == 'FSx': fsx_m = float(v[1] or 0)
        
        # 인벤토리: 프로젝트별 용량 (월말 스냅샷 고정)
        s3c = boto3.client('s3', region_name='us-west-2')
        snap_key = f'inventory-snapshots/{year}-{month}.json'
        snap_bucket = 'mogam-or-cur-stg'
        is_past_month = (int(year), int(month)) < (_kst_today().year, _kst_today().month)
        
        inv_rows = None
        if is_past_month:
            try:
                obj = s3c.get_object(Bucket=snap_bucket, Key=snap_key)
                import json as _json
                inv_rows = _json.loads(obj['Body'].read())
            except s3c.exceptions.NoSuchKey:
                pass
        
        if inv_rows is None:
            raw = run_q("SELECT split_part(key,'/',2), SUM(size) FROM cost_monitoring.s3_inventory_mogam_or WHERE split_part(key,'/',2)!='' GROUP BY 1")
            inv_rows = [[c.get('VarCharValue','') for c in r['Data']] for r in raw]
            if is_past_month:
                import json as _json
                s3c.put_object(Bucket=snap_bucket, Key=snap_key, Body=_json.dumps(inv_rows), ContentType='application/json')
        
        m = _cached_mappings()
        ntc = m.get('project_name_to_code', {})
        code_map = m.get('project_code_mapping', {})
        
        # 인벤토리 폴더 → 프로젝트 코드 변환 + 합산
        inv_by_code = {}
        for v in inv_rows:
            folder, size = v[0], int(v[1] or 0)
            code = ntc.get(folder, folder)
            # 유효 프로젝트만 (코드 매핑에 있거나 P/MI로 시작)
            if code in code_map or code.startswith(('P2', 'MI2', 'MG', 'poc:')):
                inv_by_code[code] = inv_by_code.get(code, 0) + size
        
        # 일별 배분
        s3_d, fsx_d = s3_m / days_in_month, fsx_m / days_in_month
        today = _kst_today()
        max_date = today - timedelta(days=2)
        
        totals = {}
        for day in range(1, days_in_month + 1):
            d = datetime(int(year), int(month), day).date()
            if d > max_date:
                break
            try:
                dd = get_cost_data(d.isoformat())
            except:
                continue
            rate = get_exchange_rate(d.isoformat()) or 1440
            
            # 활성 프로젝트 (EC2 비용 > 0, 유효 코드만)
            active = {}
            for pn, pc in dd.get('proj_costs', {}).items():
                c = pc.get('total_cost', 0)
                if c <= 0: continue
                code = ntc.get(pn, pn)
                if code in code_map or code.startswith(('P2', 'MI2', 'MG', 'poc:')):
                    active[code] = active.get(code, 0) + c
            if not active: continue
            
            # S3: 인벤토리 전체 프로젝트에 용량 비율로 배분
            t_inv = sum(inv_by_code.values())
            t_ec2 = sum(active.values())
            
            # S3 — 인벤토리 용량 비율 (EC2 활성 여부 무관)
            for code, sz in inv_by_code.items():
                if code not in totals:
                    totals[code] = {'s3_usd': 0, 's3_krw': 0, 'fsx_usd': 0, 'fsx_krw': 0}
                s3 = s3_d * (sz / t_inv) if t_inv > 0 else 0
                totals[code]['s3_usd'] += s3
                totals[code]['s3_krw'] += s3 * rate
            
            # FSx — EC2 활성 프로젝트 비용 비율
            for code, ec2 in active.items():
                if code not in totals:
                    totals[code] = {'s3_usd': 0, 's3_krw': 0, 'fsx_usd': 0, 'fsx_krw': 0}
                fsx = fsx_d * (ec2 / t_ec2) if t_ec2 > 0 else 0
                totals[code]['fsx_usd'] += fsx
                totals[code]['fsx_krw'] += fsx * rate
        
        return jsonify({
            's3_monthly_usd': round(s3_m, 2),
            'fsx_monthly_usd': round(fsx_m, 2),
            'days_calculated': min(days_in_month, (max_date - datetime(int(year), int(month), 1).date()).days + 1),
            'project_storage': sorted([
                p for p in [
                    {'code': c, 'description': code_map.get(c, ''),
                     's3_usd': round(v['s3_usd'], 2), 's3_krw': round(v['s3_krw']),
                     'fsx_usd': round(v['fsx_usd'], 2), 'fsx_krw': round(v['fsx_krw']),
                     'storage_total_krw': round(v['s3_krw'] + v['fsx_krw']),
                     'inventory_tb': round(inv_by_code.get(c, 0) / 1099511627776, 2)}
                    for c, v in totals.items()
                ] if p['storage_total_krw'] > 0 or p['inventory_tb'] > 0
            ], key=lambda x: -x['storage_total_krw'])
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def warmup_cache():
    ...
    def _warmup():
        ...
        for ym in [f"{today.year}/{today.month:02d}", f"{(today.replace(day=1) - timedelta(days=1)).year}/{(today.replace(day=1) - timedelta(days=1)).month:02d}"]:
            try:
                requests.get(f"http://backend-cost:5000/api/cost-monitoring/monthly-costs/{ym}", timeout=300)
            except:
                pass
